...
* Custom properties (variables) define a token system for
  colors, typography, and spacing.
* All theme values are accessed via
  `var(--color-*)`, `var(--font-*)`, `var(--space-*)`.
* Light and dark themes use
  `@media (prefers-color-scheme: dark)` overriding
  the same tokens.
* Component-level CSS utility classes (`.card`,
  `.list-item`, `.h2`) are available for use
  over inline styling.
...
Conceptual interfaces (implemented in `src/repositories/types.ts`):
...
Dexie-backed implementations:
`DexieProjectRepository` and `DexieTaskRepository`
in `src/repositories/`.
...
* Project (implemented — `src/types/domain.ts`, `src/domain/entities.ts`)
* Task (implemented — `src/types/domain.ts`, `src/domain/entities.ts`)
* Subtask
* Tag
* Note
* TimeEntry
...
Business logic and utilities (10 tests in `src/domain/entities.test.ts`).
...
Database/repository behavior (9 tests in `src/repositories/repositories.test.ts`).
...
Toolchain: Vitest + Testing Library + jsdom. All 19 tests currently pass.
...
Custom properties (tokens)
 ↓
Utility classes (.h2, .card, .list-item, etc.)
 ↓
...
Current component pattern (section):

```text
Module-level repo singleton
 ↓
useEffect → repo.getAll() → setState
 ↓
Render form + list
```

Responsive breakpoint: **768px** (desktop sidebar ≥ 768px, mobile bottom nav < 768px).
